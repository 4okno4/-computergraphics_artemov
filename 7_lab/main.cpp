...
GLfloat light_position[] = { 0.0f, 0.0f, 15.0f, 1.0f }; //позиция источника
...
glLightf(GL_LIGHT0, GL_SPOT_CUTOFF, 55); // конус для
...
float topRadius1 = 0.5f;
    ...
    drawDoubleTruncatedCone(topRadius1, bottomRadius1, height1, 23);
    glRotatef(180,1,0,0);
    glTranslatef(0.0f, 0.0f, 0.0f);
    drawDoubleTruncatedCone(topRadius2, bottomRadius2, height2, 23);
    glPopMatrix();
}
 void drawHyperboloids(){
      for (int i = 0; i < 10; i++) {
            glPushMatrix();
            glColor4f(1,0.4,0.6,i*0.1);
